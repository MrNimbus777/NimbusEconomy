...
public class YmlFile {
    ...
    public YmlFile create() {
        file = new File(NEconomy.a.getDataFolder()+"/"+ dir, name+".yml");
        if(!file.exists()) {
            File fd = new File(NEconomy.a.getDataFolder() +"/"+ dir);
            if(!fd.exists()) {
                fd.mkdir();
            }
            try {
    ...
    public static boolean exists(String dir, String name, boolean create, boolean getFromFolder){
        ...
        if((new File(NEconomy.a.getDataFolder() + "/" + dir + "/" + name+".yml")).exists() && getFromFolder){
            YmlFile file = new YmlFile(dir, name);
            file.create();
            return true;
        } return false;
    ...
    public static ArrayList<String> getDirList(String dir){
        ...
        if(new File(NEconomy.a.getDataFolder() + "/" + dir).exists()) list = new ArrayList<>(Arrays.asList(new File(NEconomy.a.getDataFolder() + "/" + dir).list()));
        return list;
    }
}
